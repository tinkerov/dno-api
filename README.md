...
В проекте есть pytest (покрытие ~80%). Чтобы их запустить, введите следующие команды

Убедитесь что путь в терминале ведет к папке проекта, потом перейдите в папку с тестами следующей командой

```bash
cd backend
```

После того как вы убедились, что путь в терминале заканчивается на папке backend, запустите тесты следующей командой
